...
export const createTask = async (req, res) => {
    try {
        ...
        await pool.query('INSERT INTO tasks (title, description, urgent, file_path, status, user_id) VALUES (?, ?, ?, ?, ?, ?)', [title, description, urgent, filePath, status, userId]);
        // ⬅️ Aqui o ajuste
        const [tasksDoUsuario] = await pool.query('SELECT * FROM tasks WHERE user_id = ?', [userId]);
...
export async function getTasks(req, res) {
    try {
        const usuarioId = req.user.id; // 🔐 ID obtido do token JWT
        const [tarefasDoUsuario] = await pool.query('SELECT * FROM tasks WHERE user_id = ?', [usuarioId]);
...
export const updateTask = async (req, res) => {
    ...
    const userId = req.user.id;
    try {
        const [result] = await pool.query('UPDATE tasks SET title = ?, description = ?, urgent = ? WHERE id = ? AND user_id = ?', [title, description, urgent, id, userId]);
